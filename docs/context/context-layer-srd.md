...
| FR-003 | **Schema profiling.** Deterministically compute row count, null ratio, distinct count, uniqueness ratio, min/max/avg, length ranges, and ranked candidate roles; partial column failures do not fail the job. | Profiles match direct SQL aggregates on the same data; a forced per-column error yields a recorded coverage gap, not a job failure |
