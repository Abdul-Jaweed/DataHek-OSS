...
export function ProgressSection({
              ...
              {state === 'done' ? (
                ...
                <span
                  className={cn(
                    ...
                    state === 'active' ? 'animate-pulse bg-current' : 'bg-current opacity-40',
                  )}
                />
              )}
              {s.label}
  ...
  return (
    <div className="border-b border-border bg-surface px-4 py-2 sm:px-6" role="status" aria-live="polite">
      <div className="mx-auto flex w-full max-w-[1060px] flex-wrap items-center gap-2">{chips}</div>
    </div>
  );
}
